    ...
    for i in range(len(list_name)):
        if list_gszzl[i] != '':
            if float(list_gszzl[i]) < float(-2.0):
                list_sum = [list_code[i],list_name[i], list_gsz[i], list_gszzl[i], list_dwjz[i]]
                important_information.append(list_sum)
    important_information.sort(key=lambda x:(x[3]),reverse=True)
...
def text_to_html():
    ...
    for i in range(len(important_information)):
        tail_html_str = '''<tr>
            ...
            important_information[i][0],important_information[i][1],important_information[i][2],important_information[i][3],important_information[i][4])
