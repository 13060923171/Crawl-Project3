...
def get_data():
    ...
    #持仓成本价
    list_cccb = [3.8008,2.5073,2.9368,3.4830,1.2221,1.5800]
    #总收益
    list_sum_sy = []
    for j in range(len(list_dwjz)):
        sy = float((float(list_dwjz[j]) - float(list_cccb[j])) / float(list_cccb[j]))
        sy = '{:.4}'.format(sy)
        sy = f"{round(float(sy) * 100,2)}%"
        list_sum_sy.append(sy)
    for i in range(len(list_name)):
        list_sum = [list_name[i], list_gsz[i], list_gszzl[i], list_dwjz[i],list_cccb[i],list_sum_sy[i]]
...
def text_to_html():
    ...
    for i in range(len(important_information)):
        tail_html_str = '''<tr>
            ...
            important_information[i][0],important_information[i][1],important_information[i][2],important_information[i][3],important_information[i][4],important_information[i][5])
...
def send_mail(receiver):
    ...
    <table width="90%" border="1" cellspacing="0" cellpadding="4" bgcolor="#cccccc" class="tabtop13">
        <tr>
        <th colspan="6" class="btbg titfont">
        </tr>
